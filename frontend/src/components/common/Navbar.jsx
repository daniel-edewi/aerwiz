...
import { Plane, Menu, X, ChevronDown, Bell, User, LogOut, Settings, FileSearch, Home, Luggage } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navLinks = [
    { label: 'Flights', path: '/', icon: Plane },
    { label: 'Manage Booking', path: '/manage-booking', icon: FileSearch, action: () => {
      navigate('/');
      setTimeout(() => {
    ...
    { label: 'Baggage', path: '/baggage', icon: Luggage },
    { label: 'Price Alerts', path: '/alerts', icon: Bell, requiresAuth: true },
  ];
              ...
              return (
                <button key={link.label}
                  ...
                  className={`flex items-center space-x-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    isActive(link.path) ? 'text-blue-600 bg-blue-50' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'
                  }`}>
                  ...
                  <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
                    ...
                    <div className="py-1">
                      {[
                        ...
                        { label: 'Account Settings', icon: Settings, path: '/dashboard', tab: 'profile' },
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 py-3 space-y-1">
            {navLinks.map(link => {
              ...
              return (
                <button key={link.label}
                  onClick={() => { link.action ? link.action() : navigate(link.path); setMenuOpen(false); }}
            ...
            {!isAuthenticated && (
              <div className="flex space-x-2 pt-2 border-t border-gray-100">
                <button onClick={() => { navigate('/login'); setMenuOpen(false); }}
                  className="flex-1 text-center py-2.5 text-sm font-medium text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50">Login</button>
                <button onClick={() => { navigate('/register'); setMenuOpen(false); }}
                  className="flex-1 text-center py-2.5 text-sm font-medium text-white bg-blue-600 rounded-xl hover:bg-blue-700">Sign Up</button>
              </div>
            )}
          </div>
        )}
